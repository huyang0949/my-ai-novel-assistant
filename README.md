...
项目当前面向浏览器运行。启动前后端开发服务后，访问 `http://localhost:5173` 即可进入小说创作工作台。
...
- Monorepo 拆分（pnpm workspace），共享包、服务端和客户端可独立构建
...
- 项目入口集中到 README 与 Web 小说创作工作台，暂时不再维护独立的公开介绍页和在线文档门户。
- 本地安装、启动和构建命令聚焦客户端与服务端，减少与小说创作主链无关的维护项。
- 使用说明、生产链文档和故障排查资料继续保留在仓库 `docs/` 中。
