...
- 项目暂时收起独立的公开介绍页和在线文档门户，仓库入口集中到 README 与 Web 小说创作工作台。
- 本地开发不再需要维护或构建独立介绍站，安装、启动和构建命令聚焦客户端与服务端。
- 使用说明、生产链文档和故障排查资料继续保留在仓库 `docs/` 中，可直接从源码仓库查阅。
...
- 项目后续统一通过 Web 端运行，移除桌面应用安装、自动更新和本地宿主维护路径，首次安装与启动更直接。
