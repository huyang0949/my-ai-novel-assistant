...
角色图、封面和拆书外形图都会产生可复用的图片，统一保存在 `ImageAsset` 中。让每个页面各自查询图片会使作者无法统一浏览已有视觉资产，也让“选择一张图片作为参考图”的能力反复实现。
...
- 原始图片和业务事实仍归 `ImageAsset` 来源模块所有；资源目录只索引，不拥有文件生命周期。
...
- 资源目录不负责创建、删除或迁移来源图片，避免把展示层和文件生命周期耦合。
