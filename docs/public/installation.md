...
这篇文档帮助你准备 Web 端 AI 小说创作工作台，并确认模型、存储和知识库能力是否可用。
...
3. 运行 `pnpm dev`。
4. 打开 `http://localhost:5173`，配置模型并创建测试小说。
...
开发环境默认把小说、任务状态、配置和本地数据库保存在服务端工作区。你可以在故障排查文档中查看日志和数据备份建议。
...
开发者可以从源码运行客户端和服务端。请先确认 Node.js 和 pnpm 版本满足仓库要求，再安装依赖。
